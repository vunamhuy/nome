    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-html" class="form-horizontal">
		  ...
		  <div class="form-group">
            <label class="col-sm-2 control-label" for="input-status">Chọn kiểu hiển thị</label>
            <div class="col-sm-10">
			  <?php 
			    $group_class_array = array(
					'normal'=> 'normal',
					'bottomFix' => 'Cố định dưới',
					'rightFix' => 'Cố định phải',
					'leftFix' => 'Cố định trái'
				);
			  ?>
              <select name="group_class" id="input-status" class="form-control">
                <?php if ($group_class_array) { ?>
				<?php foreach($group_class_array as $keyClass=>$class) {?>
                <option value="<?php echo $keyClass;?>" <?php if($keyClass==$group_class){echo "selected";} ?> ><?php echo $class; ?></option>
                <?php } ?>
				<?php } ?>
              </select>
            </div>
